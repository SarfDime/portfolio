...
export default function Nav() {
  ...
  const currentElement =
    useSelector((state) => state.intersection.currentElement) || ""
  ...
  const handleButtonClick = (e) => {
    ...
    if (elementToScroll) {
      if (e === "introMain"){
        ...
        scroll.animateScroll(elementToScroll, null, { speed: 500 })
        return
      }
      const scroll = new SmoothScroll()
      scroll.animateScroll(elementToScroll, null, { speed: 650 })
    }

    const buttons = document.querySelectorAll('.nav-button:not(.active)')

    buttons.forEach(button => {
      button.style.setProperty('--origin', '50% 50%')
  ...
  return (
    <nav className={isNavActive ? "nav-active" : ""}>
      <button
        {...getButtonProps("introMain", activeButton)}
        onClick={() => handleButtonClick("introMain")}
        disabled={activeButton === "introMain"}
      >
      ...
      <button
        {...getButtonProps("aboutMain", activeButton)}
        onClick={() => handleButtonClick("aboutMain")}
        disabled={activeButton === "aboutMain"}
      >
        <svg viewBox="0 0 24 24">
